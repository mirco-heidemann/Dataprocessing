...
#        paste('Schadensumme erfasster Schäden [CHF]:',
...
#   ###### ###### ######
  ...
  # ///////////// ///////////// /////////////
  ## (1) SCHÄDEN GEOREFERENZIEREN
  
  dir.in.path <- 'J:/Naturgefahren/FG2_Datenanalysen/Portfolio gvz/gebäudebestand gvz/2017/data georef/gebaeudebestand shapes - AV umrisse/'
  ...
  ## (Räumliche) Zuordnung Bestand -> Schaden (ohne geox, geoy Information) über Gebäudenummer
  ...
  popup <- paste("<strong>Schadennummer: </strong>", data.event.WGS84$schadenNr, "<br>",
                 ...
                 " ", "<br>",
                 "<strong>Gebäudenummer: </strong>", data.event.WGS84$gebNr, "<br>",
                 "<strong>Versicherungssumme [CHF]: </strong>", prettyNum(as.character(data.event.WGS84$versSum),big.mark="'"), "<br>",
                 "<strong>Gebäudezweck: </strong>", data.event.WGS84$zwCode)
  ...
  m <- leaflet(data.event.WGS84) %>% 
    ...
    #### setView(lng = 8.54226, lat = 47.37174, zoom = 11) %>%            # Zürich
