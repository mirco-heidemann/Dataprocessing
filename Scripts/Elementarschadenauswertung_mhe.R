...
title.graf <- 'Schadenmeldungen durch Unwetter vom 01.08.2017'
...
ind.dat <- which(format(data$schadDat, '%Y%m%d') >= format(event.date, '%Y%m%d')
                 & format(data$schadDat, '%Y%m%d') <= format(date.end, '%Y%m%d'))
...
# csv.file <- paste('pdf/',event.string,'.',
#                   format(event.date, '%Y%m%d'), '.csv', sep = '')
...
#                   format(event.date, '%Y%m%d'),'.pdf', sep = '')
...
# # title(main = paste(event.string,', ', format(event.date, '%d.%m.%Y'),sep=''),
# #       line = 0.5, cex.main=1)
...
## prepare the pdf-plot
pdf.file <- paste('pdf/', format(event.date, '%Y%m%d'), '.schadAnz.',
                  event.string, '.pdf', sep = '')
...
plot.gem('AnzahlSchaeden', breaks, data.gem)
title(main = title.graf, line = 0.5, cex.main=1)
text(txt.x, txt.y, paste('Anzahl erfasster Schäden pro Gemeinde',
